...
export const SquarexConfig: React.FC = () => {
  ...
  useEffect(() => {
    if (gameState.players.length < gameState.config.count) {
      let newPlayers: { name: string; correct: number; incorrect: number; odd: number; even: number; group: string }[] = [];
      for (let i = 1; i <= gameState.config.count - gameState.players.length; i++) {
        newPlayers.push({ name: `Player ${gameState.players.length + i}`, correct: 0, incorrect: 0, odd: 0, even: 0, group: "" });
      }
...
export const SquarexBoard: React.FC = () => {
  ...
  const correct = (playerIndex: number) => {
    setGameState(produce(gameState, draft => {
      ...
      draft.logs.unshift({ type: "squarex", variant: "correct", player: playerIndex });
    }));
  }
  ...
  const incorrect = (playerIndex: number) => {
    setGameState(produce(gameState, draft => {
      ...
      draft.logs.unshift({ type: "squarex", variant: "incorrect", player: playerIndex });
    }));
  }
