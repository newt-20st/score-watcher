...
import { getAttacksurvivalGameState, initialQuizData, AttacksurvivalGameStateProps, AttacksurvivalInitialGameState, QuizDataProps } from "../libs/state";

export const AttacksurvivalConfig: React.FC = () => {
  const [gameState, setGameState] = useState<AttacksurvivalGameStateProps>(getAttacksurvivalGameState());
  ...
  useEffect(() => {
    if (gameState.players.length < gameState.config.count) {
      let newPlayers: { name: string; score: number; correct: number; incorrect: number; group: string }[] = [];
      for (let i = 1; i <= gameState.config.count - gameState.players.length; i++) {
        newPlayers.push({ name: `Player ${gameState.players.length + i}`, score: gameState.config.n, correct: 0, incorrect: 0, group: "" });
      }
        ...
        <Flex pt={5} gap={5}>
          ...
          <Flex direction="column" flexGrow={1} gap={5}>
            <Flex>
              ...
              <FormControl>
                <FormLabel>初期ポイント <Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
            ...
            <Flex>
              <FormControl>
                <FormLabel>自分が正答<Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
                <NumberInput min={1} max={15}
                  value={gameState.config.correct.me} onChange={e => setGameState(produce(gameState, draft => { draft.config.correct.me = e as any }))} >
                  <NumberInputField />
                  <NumberInputStepper>
                    <NumberIncrementStepper />
                    <NumberDecrementStepper />
              ...
              <FormControl>
                <FormLabel>他人が正答<Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
                <NumberInput min={1} max={15}
                  value={gameState.config.correct.other} onChange={e => setGameState(produce(gameState, draft => { draft.config.correct.other = e as any }))} >
                  <NumberInputField />
                  <NumberInputStepper>
                    <NumberIncrementStepper />
                    <NumberDecrementStepper />
              ...
              <FormControl>
                <FormLabel>自分が誤答<Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
                <NumberInput min={1} max={15}
                  value={gameState.config.incorrect.me} onChange={e => setGameState(produce(gameState, draft => { draft.config.incorrect.me = e as any }))} >
                  <NumberInputField />
                  <NumberInputStepper>
                    <NumberIncrementStepper />
                    <NumberDecrementStepper />
              ...
              <FormControl>
                <FormLabel>他人が誤答<Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
                <NumberInput min={1} max={15}
                  value={gameState.config.incorrect.other} onChange={e => setGameState(produce(gameState, draft => { draft.config.incorrect.other = e as any }))} >
                  <NumberInputField />
                  <NumberInputStepper>
                    <NumberIncrementStepper />
                    <NumberDecrementStepper />
              ...
              <FormControl>
                <FormLabel>スルー<Badge colorScheme="red" mx={2}>必須</Badge></FormLabel>
                <NumberInput min={1} max={15}
                  value={gameState.config.through} onChange={e => setGameState(produce(gameState, draft => { draft.config.through = e as any }))} >
                  <NumberInputField />
                  <NumberInputStepper>
                    <NumberIncrementStepper />
                    <NumberDecrementStepper />
...
export const AttacksurvivalBoard: React.FC = () => {
  ...
  const correct = (playerIndex: number) => {
    setGameState(produce(gameState, draft => {
      ...
      draft.logs.unshift({ type: "attacksurvival", variant: "correct", player: playerIndex });
    }));
  }
  ...
  const incorrect = (playerIndex: number) => {
    setGameState(produce(gameState, draft => {
      ...
      draft.players = newPlayerState;
      draft.logs.unshift({ type: "attacksurvival", variant: "incorrect", player: playerIndex });
    }));
  }
  ...
  return (
    <Box>
      ...
      <Flex p={3} justifyContent="flex-end" gap={2} alignItems="center">
        ...
        <Button onClick={() => navigate("/config/attacksurvival")} colorScheme="teal" size="xs">設定に戻る</Button>
      </Flex>
